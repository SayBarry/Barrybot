...
$owners = ["6440962840"];  // Add owner ids here
...
if (isset($update['message']['text'])) {
    ...
    if ($message === '/users') {
        if (in_array($chat_id, $owners)) {
            ...
            $response = "<b>[×] Total users: {$freeUserCount}%0A[×] Paid users: {$paidUserCount}%0A[×] Banned users: {$banUserCount}%0A%0A[×] Bot by: @Barry_op </b>";
        } else {
